...
probe.current = 145; const rear = s.measure()
check(rear.samples.some(p => p.viscous < -.02), 'rear preset locates measured reverse flow on the initial frame')
...
for (const w of [340, 640]) {
  ...
  probe.current = 145; const changed = draw()
