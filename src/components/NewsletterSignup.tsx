...
import { getAnalyticsContext, trackEvent, trackOnce, type SignupPlacement } from '../analytics/browser'
...
export default function NewsletterSignup({ placement = 'top' }: { placement?: SignupPlacement }) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(entries => {
      if (active && entries.some(entry => entry.isIntersecting)) {
        trackOnce('newsletter_form_viewed', `form:${placement}`, { placement })
        observer.disconnect()
      }
    })
    observer.observe(form)
    return () => { active = false; observer.disconnect() }
  }, [placement, pathname, search])
  ...
  async function copyFeed() {
    ...
    try {
      ...
      void trackEvent('rss_copy', { placement }, context)
      setRss('copied')
    } catch {
      setRss('manual')
    }
  ...
  async function subscribe(event: FormEvent<HTMLFormElement>) {
    ...
    void trackEvent('newsletter_signup_started', { placement }, context)
    try {
      ...
      const response = await fetch(`${endpoint}/subscribe`, {
        ...
        body: JSON.stringify({ email: form.get('email'), website: form.get('website'), source: pathname, consent: true, placement,
          analytics: context ? { visitorId: context.visitorId, sessionId: context.sessionId, pageId: context.pageId } : false }),
      ...
      void trackEvent('newsletter_signup_failed', { placement }, context)
...
export function NewsletterIntro() {
  ...
  return <div id="newsletter"><NewsletterSignup key={pathname} placement="top" /></div>
}
