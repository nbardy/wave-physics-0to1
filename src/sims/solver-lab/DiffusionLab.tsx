...
export function createDiffusion(value: { current: number }, shape: { current: Profile }): Stepper {
  ...
  return { step() {}, draw(ctx, w, h) {
    ...
    panes(w, h).forEach((p, n) => {
      ...
      const energy = values.reduce((s, u) => s + u * u, 0) / f.initial.reduce((s, u) => s + u * u, 0)
      label(ctx, `Kinetic energy: ${(energy * 100).toFixed(0)}%`, p.x, p.y + p.h - 28, C.visc, 13, true)
...
export function DiffusionLab() {
  ...
  return <div data-lab="diffusion"><Sim animated={false} resettable={false} height={height} create={() => createDiffusion(vRef, sRef)}>
